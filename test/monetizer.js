...
const balance = async (address) =>
  parseInt(await web3.eth.getBalance(address));

contract("Monetizer", (accounts) => {
  let monetizer, wrb, witnetFee;
  let wrbOperatorInitialBalance,
    depositorInitialBalance,
    beneficiaryInitialBalance,
    monetizerInitialBalance,
    depositorLocalBalance,
    beneficiaryLocalBalance;
  ...
  describe("End-2-end", () => {
    ...
    it("should take balance away from depositor", async () => {
      assert(
        (await balance(wrbOperator)) === wrbOperatorInitialBalance,
        "wrbOperator balance has not been reduced"
      );
      assert(
        depositorInitialBalance - (await balance(depositor)) > reward,
        "depositor balance has not been reduced"
      );
      assert(
        (await balance(beneficiary)) === beneficiaryInitialBalance,
        "beneficiary balance has changed"
      );
      assert(
        (await balance(monetizer.address)) === reward,
        "monetizer hasn't escrowed the deposit"
      );
    });
    ...
    it("should have taken the Witnet fee from caller of `checkViews`", async () => {
      assert(
        (await balance(depositor)) <= depositorLocalBalance - witnetFee,
        "depositor didn't pay for Witnet fee"
      );
      assert(
        (await balance(wrb.address)) === witnetFee,
        "Witnet fee is not escrowed in Witnet Request Board"
      );
    });
