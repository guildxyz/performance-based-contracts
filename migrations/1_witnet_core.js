...
module.exports = async function (deployer, network, accounts) {
  ...
  if (network in addresses) {
    ...
    console.warn(
      `Network "${network}" is not officially supported by Witnet. A mock Witnet environment will be used.`
    );
    const WitnetDecoderLib = artifacts.require(
      artifactNames["WitnetDecoderLib"]
    );
    const WitnetParserLib = artifacts.require(
      artifactNames["WitnetParserLib"]
    );
    const WitnetRequestBoard = artifacts.require(
      artifactNames["WitnetRequestBoard"]
    );
    ...
    if (!WitnetDecoderLib.isDeployed()) {
      ...
      await deployer.link(WitnetDecoderLib, [
        WitnetParserLib,
        WitnetRequestBoard
      ]);
    }
    ...
    if (!WitnetRequestBoard.isDeployed()) {
      await deployer.deploy(
        WitnetRequestBoard,
        true,
        "0x302e342e322d747275737461626c65"
      );
      upgradeProxy = true;
    }
